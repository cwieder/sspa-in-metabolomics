...
from simulate_met_data import SimulateDatasetNamed
...
nr_set = {k: v for k, v in pathways_present.items() if
          k in ['R-HSA-109582', 'R-HSA-1237112', 'R-HSA-1266738', 'R-HSA-1483115', 'R-HSA-156582', 'R-HSA-156590',
                'R-HSA-1614603', 'R-HSA-1660662', 'R-HSA-189200', 'R-HSA-196071', 'R-HSA-211981', 'R-HSA-3296197',
                'R-HSA-352230', 'R-HSA-5619084', 'R-HSA-5683826', 'R-HSA-73614', 'R-HSA-77108', 'R-HSA-917937']}

res_dfs = []

# change effect size - log fold change
effect_size = [0, 0.2, 0.4, 0.6, 0.8, 1]
...
for e in effect_size:
    ...
    # run methods
    res = helper_functs.get_metrics(sims_dict.keys(), iter_sim, pathways_greater_than_5, 0.05, path_coverage, 0.5,
                                    "COVID19 ", testtype="mwu")
    res_dfs.append(res)

final_df = pd.concat(res_dfs, keys=effect_size)

final_df.to_csv("tempmetrics/metrics_3upreg_effect_200_o5_p005_MWU_" + str(sys.argv[1]) + ".csv")