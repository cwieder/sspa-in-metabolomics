...
for e in effect_size:
    ...
    goldman_sim_iter = SimulateDatasetNamed(goldman_data.data_proc.iloc[:, :-2],
                                            ...
                                            "COVID19 ",
                                            pathways_present, e, upreg_paths=random_pathways)
    ...
    # run methods
#     r_df, r_sum = helper_functs.get_rankings_multivariate([*upreg_paths, *downreg_paths], iter_sim,
#                                                           pathways_present)
    
    r_df, r_sum = helper_functs.get_rankings_univariate([*upreg_paths, *downreg_paths], iter_sim,
                                                          pathways_present, "COVID19 ")
    res_dfs.append(r_df)
    snr_all.append(r_sum)

avg_rankings_all = np.array(snr_all)
res_df = pd.DataFrame(avg_rankings_all, index=effect_size, columns=res_dfs[1].columns)

res_df.to_csv("tempnoise/ANR_effectsize_3upreg_uni_" + str(sys.argv[1]) + ".csv")
